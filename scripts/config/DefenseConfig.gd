class_name DefenseConfig
extends Resource

@export var guard_distance: float = 78.0
@export var guard_arrival_radius: float = 38.0
@export var guard_stop_radius: float = 14.0
@export var guard_acceleration: float = 860.0
@export var guard_deceleration: float = 980.0
@export var guard_deadband_radius: float = 22.0
@export var contest_radius: float = 110.0
@export var block_radius: float = 88.0
@export var pressure_radius: float = 72.0
@export var stationary_turnover_time: float = 1.4
@export var steal_check_interval: float = 0.35
@export var help_recovery_strength: float = 0.22
