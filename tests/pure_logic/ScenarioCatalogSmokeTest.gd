...
func get_test_id() -> String:
	return "scenario_catalog_required_entries"
...
func run_case(harness: Object) -> TestResult:
	...
	typed_harness.load_scenarios()
	var missing := PackedStringArray()
	for required_id in typed_harness.config.required_scenarios:
		if not typed_harness.loaded_scenarios.has(required_id):
			missing.append(required_id)
	if missing.is_empty():
		return TestResult.passed(get_test_id(), get_category(), PackedStringArray([
			"All required scenario definitions are present.",
		]))
	return TestResult.failed(get_test_id(), get_category(), PackedStringArray([
		"Missing scenarios: %s" % ", ".join(missing),
	]))
